...
import { faPaintBrush, faUserTie, faCalendarAlt, faFileAlt } from '@fortawesome/free-solid-svg-icons';
...
export default () => {
    const features = [
        ...
        {
            icon: farCalendarAlt, // Using the renamed variable
                        ...
                        {features.map((item, idx) => (
                            <li key={idx} className={`feature-${idx + 1} space-y-3 py-8 lg:px-12 sm:py-0`}>
                                <div className="w-12 h-12 border text-indigo-600 rounded-full flex items-center justify-center">
                                    <FontAwesomeIcon icon={item.icon} /> {/* Render the icon using FontAwesomeIcon */}
                                </div>
